...
int main()
{
 ...
 while(T--)
 {
   ...
   for(int i = 0; i < maxn; i++)
     for(int j = 0; j < 30; j++)
             { C[i][j] = 1000; B[i][j] = -1000;}
   int p;
   for(int i = 1; i <= n; i++)
     {scanf("%d",&p); in[i] += p; in[s] -= p;}
   for(int i = 1; i <= m; i++)
     {scanf("%d",&p); in[i+n] -= p; in[t] += p;}
   ...
   if(r == 0)
     {
         if(q == 0)
             for(int i = 1; i <= n; i++)
                for(int j = 1; j <= m; j++)
                    if(ch == '=')  B[i][j] = C[i][j] = v; 
                    else if(ch == '<') C[i][j] = min(C[i][j],v-1);
                    else B[i][j] = max(B[i][j],v+1);
         else for(int i = 1; i <= n; i++)
                    if(ch == '=')  B[i][q] = C[i][q] = v; 
                    else if(ch == '<') C[i][q] = min(C[i][q],v-1);
                    else B[i][q] = max(B[i][q],v+1);
     }
  else 
    {
        if(q == 0)
          for(int j = 1; j <= m; j++)
	  if(ch == '=') B[r][j] = C[r][j] = v;
	  else if(ch == '<') C[r][j] = min(C[r][j],v-1);
	  else B[r][j] = max(B[r][j],v+1);
        else 
           if(ch == '=')  B[r][q] = C[r][q] = v; 
           else if(ch == '<') C[r][q] = min(C[r][q],v-1);
           else B[r][q] = max(B[r][q],v+1);
    }
  }
